...
func TestGitLs(t *testing.T) {
	output, _, ok := Ls("git@github.com:floeit/floe.git", "", "", "")
	if !ok {
	...
	output, _, ok = Ls("git@github.com:floeit/floe.git", "master", "", "")
	if !ok {
		t.Error("git ls failed")
	}
	if len(output.Hashes) != 1 {
		t.Error("got more than one master")
	}
}
